...
This is a function that prints all the elements of a ```C listint_t``` list.

* Prototype: ```C size_t print_listint(const listint_t *h);```
* Return: the number of nodes
* If ```C str``` is ```C NULL```, print ```C [0] (nil)```
* Did I say it is forbidden? This function employs ```C printf``` 😋
...
This is a function that returns the number of elements in a linked ```C listint_t``` list.

* Prototype: ```C size_t listint_len(const listint_t *h);```
...
This is a function that adds a new node at the beginning of a ```C listint_t``` list.

* Prototype: ```C listint_t *add_nodeint(listint_t **head, const int n);```
* Return: the address of the new element, or ```C NULL``` if it failed
* ```C str``` needs to be duplicated
* Did I say it is forbidden? This function employs ```C strdup``` 😋
...
This a function that adds a new node at the end of a ```C listint_t``` list.

* Prototype: ```C listint_t *add_nodeint_end(listint_t **head, const int n);```
* Return: the address of the new element, or ```C NULL``` if it failed
...
This is a function that frees a ```C listint_t``` list.

* Prototype: ```C void free_listint(listint_t *head);```
...
* Prototype: ```C void free_listint2(listint_t **head);```
* This function sets the ```C head``` to ```C NULL```
...
***To get the above output, remember to use valgrand***

## 6. Pop

This a function that deletes the head node of a ```C listint_t``` linked list, and returns the head node’s data ```C (n)```.

* Prototype: ```C int pop_listint(listint_t **head);```
* if the linked list is empty return ```C 0```
...
This is a function that returns the nth node of a ```C listint_t``` linked list.

* Prototype: ```C listint_t *get_nodeint_at_index(listint_t *head, unsigned int index);```
* where ```C index``` is the index of the node, starting at ```C 0```
* if the node does not exist, return ```C NULL```
...
This is a function that returns the sum of all the data (n) of a ```C listint_t``` linked list.

* Prototype: ```C int sum_listint(listint_t *head);```
* if the list is empty, return ```C 0```
...
* Prototype: ```C listint_t *insert_nodeint_at_index(listint_t **head, unsigned int idx, int n);```
* where```C idx``` is the index of the list where the new node should be added. Index starts at ```C 0```
* Returns: the address of the new node, or ```C NULL``` if it failed
* if it is not possible to add the new node at index```C idx```, it does not add a new node and return```C NULL```
...
This is a function that deletes the node at index ```C index``` of a ```C listint_t``` linked list.

* Prototype: ```C int delete_nodeint_at_index(listint_t **head, unsigned int index);```
* where ```C index``` is the index of the node that should be deleted. Index starts at ```C 0```
* Returns: ```C 1``` if it succeeded, ```C -1``` if it failed
...
This is a function that reverses a ```C listint_t``` linked list.

* Prototype: ```C listint_t *reverse_listint(listint_t **head);```
...
This is a function that prints a ```C listint_t``` linked list.

* Prototype: ```C size_t print_listint_safe(const listint_t *head);```
...
* Prototype: ```C size_t free_listint_safe(listint_t **h);```
...
* Prototype: ```C listint_t *find_listint_loop(listint_t *head);```
* Returns: The address of the node where the loop starts, or ```C NULL``` if there is no loop
* No use of ```C malloc```, ```C free``` or arrays 🥴
