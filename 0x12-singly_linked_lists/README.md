...
* Prototype: ```C size_t print_list(const list_t *h);```
...
* Prototype: ```C size_t list_len(const list_t *h);```
...
* Prototype: ```C list_t *add_node(list_t **head, const char *str);```
...
* Prototype: ```C list_t *add_node_end(list_t **head, const char *str);```
* Return: the address of the new element, or ```C NULL``` if it failed
* ```C str``` needs to be duplicated
* Did I say it is forbidden? This function employs ```C strdup``` 😋
...
This a function that frees a list_t list.

* Prototype:```C void free_list(list_t *head);```
...
This is a function that prints ```C You're beat! and yet, you must allow,\nI bore my house upon my back!\n``` before the ```C main``` function is executed.
...
This a 64-bit program in assembly that prints ```C Hello, Holberton```, followed by a new line.

* Did I say it is forbidden? This function employs only ```C printf``` 😋
* Did I say it is forbidden? This function employs ```C interrupts``` 😋
* Your program will be compiled using ```C nasm```and ```C gcc```:
